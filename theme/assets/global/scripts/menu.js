$(document).ready(function(){
	...
	var menuJsonStr = sessionStorage.getItem("menu"+ctx);
	...
	if(menuJsonStr){
		...
		//获取菜单数据
		$.post(ctx+"/menu/queryMenu",function(menuJson){
			...
			makeMenuActive();
			//缓存菜单
			sessionStorage.setItem("menu"+ctx,JSON.stringify(menuJson));
		})
	}
	...
	function menuNodeDomFactory(menuObj,level){
		...
		if(isGroupHead){
			...
			if(hasSubMenu){
				...
				k++;
				nodeHtml += "<a href='"+menuObj.url+"' class='nav-link no-submenu' index='"+k+"'>";
				nodeHtml += "<i class='fa "+menuObj.icon+"'></i>";
				nodeHtml += "<span class='title'>"+menuObj.text+"</span>";
			}
